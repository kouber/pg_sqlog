...
CREATE FUNCTION cache(timestamp = now(), rebuild bool = false) RETURNS name AS $$
DECLARE
  tbl name;
  cln name;
BEGIN
  ...
  BEGIN
    ...
    IF rebuild THEN
      EXECUTE FORMAT('DROP TABLE IF EXISTS @extschema@.%I CASCADE', tbl);
    ELSE
      BEGIN
        PERFORM @extschema@.expire_cache();
    ...
    EXECUTE FORMAT('
      CREATE UNLOGGED TABLE IF NOT EXISTS @extschema@.%I (LIKE @extschema@.log)',
      tbl
    );

    EXECUTE FORMAT('
      COPY @extschema@.%I FROM %L CSV',
      tbl,
      @extschema@.log_path($1)
    );
    ...
    -- switch @extschema@.today & @extschema@.yesterday
...
BEGIN
  EXECUTE FORMAT('DROP TABLE IF EXISTS @extschema@.%I CASCADE', $1);
...
CREATE FUNCTION @extschema@.expire_cache(keep interval = NULL) RETURNS SETOF name AS $$
DECLARE
  tbl name;
BEGIN
  IF keep IS NULL THEN
    keep := current_setting('@extschema@.cache_expire_interval')::interval;
  END IF;

  IF keep = '0 days' THEN
    RAISE NOTICE 'Cache expiry is disabled (@extschema@.cache_expire_interval)';
    RETURN;
  END IF;

  tbl := @extschema@.format_cache_table(CURRENT_DATE - keep);
  ...
  AND
    tablename < tbl
  LOOP
    PERFORM @extschema@.drop_cache(tbl);
    RETURN NEXT tbl;
  END LOOP;
